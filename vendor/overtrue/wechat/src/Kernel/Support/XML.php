...
class XML
{
    ...
    public static function buildUTF(
        $data,
        ...
        $attr = empty($attr) ? '' : " {$attr}";
        $xml = "<{$root}{$attr}>";
        $xml .= self::data2XmlUTF($data, $item, $id);
//        $xml .= "</{$root}>";
     ...
     * @return string
     */
    protected static function data2XmlUTF($data, $item = 'item', $id = 'id')
    {
        $xml = $attr = '';
        ...
        foreach ($data as $key => $val) {
            ...
            if ((is_array($val) || is_object($val))) {
                $xml .= self::data2XmlUTF((array) $val, $item, $id);
            } else {
                $xml .= $val;
            }
