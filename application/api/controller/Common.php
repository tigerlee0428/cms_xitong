...
class Common extends Api
{
    ...
    public function upload()
    {
        ...
        if($fileInfo['type']=='image/jpeg' || $fileInfo['type']=='image/png')
        {
            ...
            $image->thumb(220, 220 * $imageheight / $imagewidth)->save(ROOT_PATH . '/public' . $uploadDir . 'thumb_'.$splInfo->getSaveName());
        }
        if ($splInfo) {
            $params = array(
                'admin_id'    => 0,
    ...
    public function video()
    {
        ...
        $data['third_id'] = $res['third_id'];
        $data['video_img'] = $res['image'];
        $data['duration'] = $res['time'];
        $data['p_url'] = $res['p_url'];
        $data['m_url'] = $res['m_url'];
        $data['error_info'] = $res['error_desc'];
        $data['status'] = 1;
        $data['backtime'] = time();
