...
class Auth
{
     ...
     * 微信openid登录
     *
     * @param string $info  微信信息
     * @return boolean
     */
    public function wxopenidlogin($info)
    {
        $user = User::get(["openid" => $info['openid']]);
        if (!$user) {
            $ip = request()->ip();
            $time = time();
     ...
     * @return boolean
     */
    public function cardnologin($cardno)
    {
        $user = User::get(["cardnum" => $cardno]);
        if (!$user) {
            return false;
        }
        /*
