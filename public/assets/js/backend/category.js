		...
		init:function(){
		    ...
		    $table.bootstrapTable({
	            ...
	            columns: [
	                ...
	                { field: 'title',  title: __('Title') ,formatter:title},
	                { field: 'status',  title: __('IsOn'),align: 'center',formatter:status},
	                { field: 'weight',  title: __('Weight'),align: 'center'},
		    ...
		    function title(value, row, index){
		    	...
		    	var count = '(文档：'+row.counts+')';
		    	var str = '';
		    	if(row.level == 0){
		    		str += '<span style="color:#a35d26;font-weight:bold">' + value + '</span>';
		    	}else if(row.level == 1){
		    		str += '<span style="color:#a35d26;">' + value + '</span>';
		    	}else{
		    		str += '<span style="color:#444444;">' + value + '</span>';
		    	}
		    	str += '&nbsp;&nbsp;<span style="font-size:12px;color:#347ef1">[ID:'+row.id+']&nbsp;'+count+'</span>';
		    	return str;
		    }
		    //状态
		    ...
		    function operator(value, row, index){
		    	var str = '';
		    	var add = '<a href="'+myTable.add_url+'/pid/' + row.id + '/level/'+(row.level + 1)+'" class="btn btn-xs btn-success btn-add btn-dialog" data-toggle="tooltip" title="" data-table-id="table" data-row-index="'+index+'" data-button-index="1" data-original-title="' + __('AddChildCate') + '"><i class="fa fa-plus"></i>&nbsp;' + __('AddChildCate') + '</a>&nbsp;&nbsp;';
